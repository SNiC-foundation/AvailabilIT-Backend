...
export default class UserService {
   ...
   * @private
   */
  private constructFinalInfoMail(
    user: User,
    platinum: Partner[],
    gold: Partner[],
    silver: Partner[],
    ...
    const mail = new FinalParticipantInfo({
      name: user.name,
    ...
    Mailer.getInstance().send(user, mail);
  }
  ...
  async sendFinalInfoSingleUser(user: User) {
    ...
    const silver = partners.filter((p) => p.package === SponsorPackage.SILVER);

    await this.constructFinalInfoMail(user, platinum, gold, silver);
  }
  ...
  async sendFinalInfoAllUsers() {
    ...
    const pl = partners.filter((p) => p.package === SponsorPackage.PLATINUM);
    const g = partners.filter((p) => p.package === SponsorPackage.GOLD);
    const s = partners.filter((p) => p.package === SponsorPackage.SILVER);

    await Promise.all(users.map((user) => this.constructFinalInfoMail(user, pl, g, s)));
  }
}
