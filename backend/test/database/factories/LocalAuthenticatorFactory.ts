...
export default class LocalAuthenticatorFactory extends Factory<LocalAuthenticator> {
  ...
  async addAdmin(username: string, password: string, roles: Role[]): Promise<LocalAuthenticator> {
    const user = await this.userFactory.createSingle();
    user.name = username;
    user.email = username;
    user.roles = roles;
    ...
    localAuth.user = user;
    localAuth.salt = salt;
    localAuth.hash = hashPassword(password, salt);
    localAuth.verifiedEmail = true;
    await this.repo.save(localAuth);
